...
export async function POST(request: Request) {
  try {
    ...
    // Require a trusted device cookie for performing admin actions
    const { cookies: cookieStore } = (await import('next/headers'))
    const cookieObj = await cookieStore()
    const deviceTokenCookie = cookieObj.get('admin_device')?.value || null
    ...
    if (updateErr) {
      console.error('confirm-registration: failed to update registration', updateErr)
    ...
    try {
      ...
      await supabase.from('admin_action_logs').insert({ action_type: 'admin_device_registered_via_qr', performed_by: currentUser.id, target_user_id: currentUser.id, ip_address: ip, user_agent: ua, metadata: { registrationId: reg.id, deviceId: device.id } })
    } catch (err) {
      console.error('confirm-registration: failed to log action', err)
    }

    return NextResponse.json({ success: true })
  } catch (err) {
    console.error('Error in confirm-registration:', err)
