...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { data: colCheck } = await supabase
      ...
      .select('column_name')
      .eq('table_name', 'ensembles')
      .eq('column_name', 'archived')

    if (!colCheck || colCheck.length === 0) {
      return NextResponse.json({ error: "Database missing 'archived' column on 'ensembles'. Run the latest migrations (see scripts/000-complete-setup.sql) to add it." }, { status: 500 })
    }

    const { error } = await supabase.from('ensembles').update({ archived }).eq('id', id)
    if (error) return NextResponse.json({ error: error.message }, { status: 500 })

    return NextResponse.json({ success: true, archived })
  } catch (err) {
    console.error('Error archiving ensemble:', err)
