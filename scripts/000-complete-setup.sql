...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns WHERE table_name='admin_roadmap' AND column_name='subcategory'
  ) THEN
    ALTER TABLE public.admin_roadmap ADD COLUMN subcategory TEXT;
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns WHERE table_name='admin_roadmap' AND column_name='category'
  ) THEN
    ALTER TABLE public.admin_roadmap ADD COLUMN category TEXT;
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns WHERE table_name='admin_roadmap' AND column_name='due_date'
  ) THEN
    ALTER TABLE public.admin_roadmap ADD COLUMN due_date DATE;
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns WHERE table_name='admin_roadmap' AND column_name='completed_at'
  ) THEN
    ALTER TABLE public.admin_roadmap ADD COLUMN completed_at TIMESTAMPTZ;
  END IF;
END$$;
...
CREATE POLICY "Staff can view roadmap" ON public.admin_roadmap
  ...
  USING (
    EXISTS (
      SELECT 1 FROM public.users u WHERE u.id = auth.uid() AND u.role IN ('admin', 'staff', 'superadmin')
    )
  );
